...
export default function UpdateModal({setUpdateModalVisible, refreshData, currentlyUpdating}) {
    ...
    useEffect(() => {
      setCurrentId(currentlyUpdating[0].id);
      setFirstName(currentlyUpdating[0].first_name);
      setLastName(currentlyUpdating[0].last_name);
      setEmail(currentlyUpdating[0].email);
      setJobTitle(currentlyUpdating[0].job_title);
      setBirthDate(currentlyUpdating[0].birth_date);
      setHireDate(currentlyUpdating[0].hire_date);
      setPhoneNumber(currentlyUpdating[0].phone_number);
      setSalary(currentlyUpdating[0].salary);
    }, [currentlyUpdating])
    ...
    const isValid = (firstName, lastName, email, jobTitle, birthDate, hireDate, phoneNumber, salary) => {
        if (!/^[1-9]\d*$/.test(currentId)) { console.log("id invalid"); return false;}
    ...
    return (
        <div className="h-screen w-screen flex bg-black fixed top-0 left-0 backdrop-blur bg-opacity-50 text-white">
            <div className="flex flex-col gap-5 m-auto bg-zinc-500 p-16 rounded">
                <h1 className="text-xl mb-2 font-bold">Update Employee</h1>
                ...
                <button onClick={areInputsValid ? handleSubmit : null} className={areInputsValid ? "p-2 bg-green-400 hover:bg-green-500 transition-all rounded font-bold m-auto" : "p-2 bg-zinc-600 text-zinc-400 rounded font-bold m-auto cursor-default"}>Update</button>
                    <button onClick={() => setUpdateModalVisible(false)} className="p-2 bg-red-400 hover:bg-red-500 transition-all rounded font-bold m-auto">Cancel</button>
                </div>
            </div>
        </div>
    )
